...
<html>
<head>
    <title>Revenue by Room Type and Payment Method</title>
    <style>
        body {
			...
			<div class="content container-fluid">
				<div class="page-header">
					<div class="row align-items-center">
						<div class="col">
							<h3 class="page-title mt-5">Revenue by Room Type and Payment Method</h3> 
                        </div>
					</div>
                    ...
                    <table id="revenueTable">
                        ...
                        $query = "SELECT r.room_type, p.select_payment, r.room_price, ch.charge_amount, SUM(ch.charge_amount) AS total_revenue
                        FROM reservation res
                        INNER JOIN transcript t ON res.reservation_id = t.reservation_id
                        INNER JOIN room r ON t.room_id = r.room_id
                        INNER JOIN charge ch ON res.reservation_id = ch.reservation_id
                        INNER JOIN payment p ON ch.payment_method_id = p.payment_method_id
                        WHERE MONTH(res.reservation_date) = MONTH(CURDATE())
                        GROUP BY r.room_type, p.select_payment";
                        ...
                        while ($row = mysqli_fetch_assoc($result)): 
							if ($previous_room_type !== $row['room_type']):
								...
								?>
								<tr>
									<td rowspan="2"><?php echo $row['room_type']; ?></td>
									<td><?php echo $row['select_payment']; ?></td>
									<td><?php echo $row['charge_amount']; ?></td>
